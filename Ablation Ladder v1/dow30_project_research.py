...
from dow30_research_support import (
    ...
    _serialize_json,
    select_best_artifact_by_robust_score,
)
...
def run_research_gate(
    ...
    audit = audit_dataset_integrity(
        df=df,
        feature_cols=build_feature_ladder(DEFAULT_FEATURE_GROUPS)["full"],
    ...
    folds = generate_walk_forward_folds(df, **PROJECT_PROTOCOL)
...
def build_callback_based_runner(
    ...
    def run_fold(
        ...
        if isinstance(candidates, pd.DataFrame) and not candidates.empty:
            scored_candidates, robust_selected = select_best_artifact_by_robust_score(
                candidates,
                **score_config,
            )
...
def run_project_ablation(
    ...
    gate = run_research_gate(df, output_dir=output_dir)
    fold_runner = build_callback_based_runner(train_and_select_fn, PROJECT_SELECTION_CONFIG)
